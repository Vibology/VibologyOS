...
- **Library hybrid architecture (approved 2026-02-13):** Three-layer system decided. ✓ The Athenaeum holds 200 source PDFs (Tier 1 & 2 complete). Layer 1: PDFs remain as authoritative sources. Layer 2: Correspondence tables (Rosetta Stones) map cross-system connections. Layer 3: Synthesis documents provide Observatory contextual content with clickable citations to source PDFs (page number + highlighted text). Citations use `source://` URL scheme → Observatory opens PDF viewer at cited passage with search highlighting. Ready to begin implementation. See `PLAN - Library Hybrid Architecture.md` for complete architecture.
